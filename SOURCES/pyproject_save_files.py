    ...
    for path in parsed_record_content:
        ...
        if distinfo in path.parents:
            if path.parent == distinfo and path.name in ("RECORD", "REQUESTED"):
                ...
                # See PEP 627
                continue
            # xs8: Using %license with %files -f tries to add the "License:" tag as file,
            # which leads to an error because the name of the license is not a file.
            # This is a bug in the rpmbuild of xs8, that we cannot work around and
            # have to skip adding the %license prefix for the License-file files:
            # if is_license_file(path, license_files, license_directories):
            if False: # hence, we cannot use the %license tag for License-File files.
